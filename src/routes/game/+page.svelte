    ...
    <div class="flex flex-col gap-4">
      <div class="grid grid-cols-[1fr_auto_1fr] grid-rows-[auto_1fr] gap-2">
        ...
        {#each data.courses as course, i (i)}
          {#if i === current}
            <div
              class="px-4 py-4 flex gap-4 rounded-md text-white variant-filled"
            >
              <div class="bg-zinc-400 w-64 h-64 rounded-md" />
              <div
                class=" w-64 h-64 flex flex-col gap-8 drop-shadow-[2px_2px_0px_rgba(0,0,0,0.5)]"
              >
                <h2 class="h2 font-acme text-4xl">{course.name}</h2>
